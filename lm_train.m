...
%                                                               %
% Assignemnt 2.                                                 %
% part2. Compute N-gram Counts                                  %
%                                                               %
% lm_train.m                                                    %
%                                                               %
% Created by Seungkyu Kim on Mar 10th, 2016                     %
...
for iFile=1:length(DD)
  ...
  for l=1:length(lines)
	...
	for i=1:length(words)
		...
		% iterate up to the second last word from the words
		if i ~= length(words)

			% check word exists in unigram, add if it new word
			if ~isfield( LM.uni, words{i} )
				LM.uni.( words{i} ) = 1;
		
			% word exists, so add count +1
			else
				LM.uni.( words{i} ) = LM.uni.( words{i} ) + 1;
			end

			% check wrod exists in bigram, add if it new word
			if ~isfield( LM.bi, words{i} )
				LM.bi.( words{i} ) = struct();
			end

			% check follwed word is exists, add if it is new bigram
			if ~isfield(LM.bi.( words{i} ), words{ i+1 } )
				LM.bi.( words{i} ).( words{ i+1 } ) = 1;
		
			% bigram already exists, so add count + 1
			else
				LM.bi.( words{i} ).( words{ i+1 } ) = LM.bi.( words{i} ).( words{ i+1 } ) + 1;
			end
		
		% add last word to unigram from words
		else
